...
export const api = {
  registraAbastec: async (dados: {
    ...
    if (!resp.ok) {
      // Isso aqui vai capturar a fofoca inteira que o backend tentou nos contar
      const erroRealDoBackend = await resp.text();
      console.log("MOTIVO DA RECUSA DO BACKEND:", erroRealDoBackend);
